...
<div id="viewContainer">
	<form method="post">
		...
		<table class="reviewTable">
			<tbody>
				<tr>
					...
					<td>${reviewDto.rv_title}</td>
				</tr>
				<tr>
					<th>작성자</th>
					<td>${reviewDto.m_name}</td>
				</tr>
				<tr>
					<td colspan="2">
						<pre><span style="font-weight:700;">작성일</span> ${reviewDto.rv_regDate}      <span style="font-weight:700;">조회수</span> ${reviewDto.rv_hit}</pre>
					</td>
				</tr>
				<tr>
					<td class="reviewContent" colspan="2">${reviewDto.rv_content}</td>
				</tr>
				<c:set var="uploadFileNames" value="${fn:split(reviewDto.rv_filename,'^')}" />
				<tr>
					<td colspan="2">
						<c:forEach var="uploadFileName" items="${uploadFileNames}">
							<img alt="${uploadFileName}" src="resources/storage/${uploadFileName}">
							<br /><br />
						</c:forEach>
					</td>
				</tr>
		...
		<table align="right">
			<tbody>
				<tr>
					<td>
						<c:if test="${loginDto.m_no == reviewDto.rv_m_no}">
							<input class="viewPageBtn" type="button" value="수정하기" onclick="reviewUpdatePage(this.form)" />
							<input class="viewPageBtn" type="button" value="삭제하기" onclick="frv_reviewDelete(this.form)" />
						</c:if>
						<input class="viewPageBtn" type="button" value="목록보기" onclick="location.href='reviewListPage.do?page=${page}'" />
					</td>
				</tr>
			</tbody>
		</table>
		...
		<input type="hidden" name="rv_title" value="${reviewDto.rv_title}">
		<input type="hidden" name="rv_content" value="${reviewDto.rv_content}">
		<input type="hidden" name="rv_filename" value="${reviewDto.rv_filename}" />
		<input type="hidden" name="page" value="${page}" />
		<c:set var="testAdmin" value="" />
		<!-- 로그인한사람이 관리자인지 테스트 -->
		<c:if test="${fn:contains(testAdmin, '관리자')}">
		</c:if>
	</form>
								...
								<c:if test="${replyDto.r_delete eq 0}">
									<td colspan="2">
										...
										<c:if test="${loginDto.m_no == replyDto.r_m_no}">
											...
											<button value="삭제" class="replyBtn" onclick="frv_replyDelete(this.form)">
												<i class="far fa-trash-alt"></i>
											</button>
										</c:if>
										<!-- hidden -->
										<input type="hidden" id="r_no" name="r_no" value="${replyDto.r_no}" />
										<input type="hidden" name="r_rv_no" value="${reviewDto.rv_no}" />
								...
								<td colspan="2">
									<div style="display: none;" id="edit_content${replyDto.r_no}">
										<div style="display: flex;">
											...
											&nbsp;&nbsp;
											<button class="replyBtn" style="width: 100px; height: 30px; line-height: 100%" onclick="frv_replyUpdate(this.form)">수정</button>
										</div>
									</div>
								</td>
							</tr>
								...
								<td colspan="2">
									<div style="display: none;" id="rereply_content${replyDto.r_no}">
										<div style="display: flex;">
											...
											&nbsp;&nbsp;
											<button class="replyBtn" style="width: 100px; height: 30px; line-height: 100%" onclick="frv_rereplyInsert(this.form)">
												<i class="far fa-comment-dots"></i>
											</button>
										</div>
									</div>
	...
	<form id="replyWrite" action="r_replyInsert.do" method="post">
...
<script>
	function reviewUpdatePage(f) {
		f.action = 'reviewUpdatePage.do';
		f.submit();
	}
	function frv_reviewDelete(f) {
	...
	function frv_replyDelete(f) {
		f.action = 'r_replyDelete.do';
		f.submit();
	}
	function frv_replyUpdate(f) {
		f.action = 'r_replyUpdate.do';
		f.submit();
	}
	function frv_rereplyInsert(f) {
		f.action = 'r_rereplyInsert.do';
		f.submit();
	}
</script>
